...
class ROSCloudSubBridge(object):
    ...
    def _receive_message(self,message):
        ...
        for i in range(len(message['transforms'])):
            ...
            self.t[-1].header.stamp.secs = message['transforms'][i]['header']['stamp']['secs']
            self.t[-1].header.stamp.nsecs = message['transforms'][i]['header']['stamp']['nsecs']
...
if __name__ == '__main__':
    ...
    a=ROSCloudSubBridge(ip='83.212.96.15', port="8063", local_topic_name="/tf_static",remote_topic_name="/tf_static", message_type="tf2_msgs/TFMessage",file=file)
    a.start()
