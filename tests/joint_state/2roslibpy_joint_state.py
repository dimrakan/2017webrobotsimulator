    ...
    def _receive_message(self,message):
           ...
           try:
            ...
            for i in range(len(message['name'])):
             msg.name.append(message['name'][i])
            for i in range(len(message['position'])):
             msg.position.append(message['position'][i])
            for i in range(len(message['velocity'])):
             msg.velocity.append(message['velocity'][i])
            for i in range(len(message['effort'])):
             msg.effort.append(message['effort'][i])
            self._rosPub=rospy.Publisher(self._local_topic_name, JointState, queue_size=10)
...
if __name__ == '__main__':
    a=ROSCloudSubBridge(ip='83.212.96.15', port="8018", local_topic_name="/local/robot/joint_states",remote_topic_name="/joint_states", message_type="sensor_msgs/JointState")
    a.start()
